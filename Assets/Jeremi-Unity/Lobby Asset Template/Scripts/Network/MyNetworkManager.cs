...
{
// This is main network script. Here we define our list of players, what happens when we a player join the server,
// How we host the server etc.
     [SerializeField] private GameObject DisconnectPanel = null; // This panel becomes visible when we stop hosting the server/leave the lobby
...
// How they join/disconnect from the game, adding new players to the player list      
    ...
    {    
        ...
        DisconnectPanel.SetActive(true); // when we stop hosting the server, show this information to other players
    }
    ...
    // How do we add our players do the list of players
    ...
    {   
        players.Clear();
        DisconnectPanel.SetActive(true); // when a client gets disconnected, show him the notification
    }
